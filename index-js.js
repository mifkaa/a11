...
// const timeIframe = document.querySelector('#timetable iframe');
// const timeButtonReact = document.querySelector('#timetable .buttonReact');

// timeButtonReact.addEventListener('click', () => {
//     timeIframe.classList.toggle('small');
// })
...
const soc = document.querySelectorAll('#contacts .soc svg');
soc.forEach(element => {

    element.addEventListener('click', () => {
        const socUrl = element.getAttribute('data-soc');
...
partQuestions.forEach(element => {
    ...
    element.addEventListener('click', () => {
        element.classList.toggle("open")
        // const sd =descriptionCard.offsetHeight;
        if (element.classList.contains('open')) {
            element.style.height = parseInt(element.dataset.height) + answer.offsetHeight + 'px';
            // console.log(element.dataset.height);
            // element.style.height = element.offsetHeight + answer.offsetHeight + 'px';

            // element.style.height = 111 + answer.offsetHeight + 'px';
            // element.style.height = '750px';
        }
        else {
            element.style.height = parseInt(element.dataset.height) + 'px';
...
const fotterCopyright = document.querySelector('footer .copyright');
const fotterMem = document.querySelector('footer img');
    fotterCopyright.addEventListener('click', () => {
        fotterMem.style.display = 'block';
    })
